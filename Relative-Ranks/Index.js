...
let findRelativeRanks = function (score) {
    let rank = [];
    let medalistStrings = ["Gold Medal", "Silver Medal", "Bronze Medal"];
    let sortedScore = [...score].sort((a, b) => b - a);

    for (let i = 0; i < score.length; i++) {
        let index = sortedScore.indexOf(score[i]);
        let medalist = medalistStrings[index];
        rank.push(medalist === undefined ? (index + 1).toString() : medalist);
    }
